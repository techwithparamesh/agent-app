...
export async function executeAnthropicAction(input: AnthropicExecuteInput): Promise<any> {
  ...
  if (actionId === 'message' || actionId === 'vision') {
    ...
    // Vision: we accept imageUrl and attach it when provided
    const imageUrl = config.imageUrl != null ? String(config.imageUrl) : '';
    ...
    return {
      model,
      text: extractTextFromMessageResponse(data),
      raw: data,
    };
  }
